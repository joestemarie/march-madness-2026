...
make download-kaggle          # Download Kaggle CSVs (needs KAGGLE_API_TOKEN in .env)
...
```bash
make forecast                 # Full pipeline: ingest Kalshi → predict → edges → allocate
make execute                  # Dry run — preview Kalshi orders
make execute-live             # Place real orders (prompts per-order confirmation)
```
...
make ingest-kalshi            # Pull latest Kalshi market prices (public, no auth)
...
- **Forecasting:** Python scripts in `forecasting/` read model + Kalshi prices, output bet sheet
- **Execution:** `forecasting/execute.py` places orders on Kalshi via authenticated API
...
- Current-year seeds live in `dbt_project/seeds/manual_seeds.csv` (keyed on `kenpom_team_name`)
- Kalshi market data is public; trading requires `KALSHI_ACCESS_KEY` + `KALSHI_PRIVATE_KEY_PATH` in `.env`
